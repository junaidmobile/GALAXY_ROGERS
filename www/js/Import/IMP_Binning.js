...
function SaveForwardDetails() {
    ...
    if (IGMno == '') {
        errmsg = "Flight No. could not be found.</br>";
        $.alert(errmsg);
        return;
    }
...
function SaveForwardDetailsForGHA() {
    ...
    var inputXML = '<Root><AWBNo>' + AWBNo + '</AWBNo><HouseNo>' + SelectedHawbNo + '</HouseNo><FlightSeqNo>' + IgmId + '</FlightSeqNo><LocCode>' + location + '</LocCode><LocId>-1</LocId><NOP>' + BinnPckgs + '</NOP><Volume>' + volume + '</Volume><UserId>' + window.localStorage.getItem("UserID") + '</UserId><AirportCity>' + AirportCity + '</AirportCity></Root>';
...
function AddLocation() {
    ...
    str += '<input id="txtLocation_' + LocCount + '" class="form-control" type="text" maxlength="20"  style="text-align:right;">'
    ...
    str += '</div>'
    str += '<div class="form-group col-xs-6 col-sm-6 col-md-6" style="padding-left: 0px;">'
    str += '<label id="vol_' + LocCount + '" for="txtVol_" class="control-label">Volume</label>'
    str += '<input id="txtVol_' + LocCount + '" class="form-control" type="number" style="text-align:right;" max="9999999">'
    str += '</div>'
    str += '</div>'
